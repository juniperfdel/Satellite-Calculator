...
def customDefault(inObj):
	if isinstance(inObj, (datetime, date)):
		return inObj.isoformat()
	return inObj
...
def solveForMoonPath(startDT, numOfDays, earthObserver):
	moonPathDict = {'start_day':startDT,"num_of_days":numOfDays}
	counter = 0
	moonPath = {}
	for dayNum in tqdm(range(numOfDays)):
		local_future_day = startDT + timedelta(days=dayNum)
		local_future_day_str = datetime.strftime(local_future_day.date(),"%Y-%m-%d")
		moonPath[local_future_day_str] = {}
		for halfHourNum in range(48):
			local_future = local_future_day + timedelta(minutes=halfHourNum*30)
			local_future_str = datetime.strftime(local_future, "%Y-%m-%d_%H:%M")
			moonFrac, moonAlt, moonAz = solveForMoonPosition(local_future, earthObserver)
			moonPath[local_future_day_str][local_future_str] = {"moonFrac":moonFrac,"moonAlt":moonAlt,"moonAz":moonAz}
	moonPathDict['moonPath'] = moonPath
	return moonPathDict
...
def isInDay(currentDT, sunTimeDict):
	...
	riseDT = curSunD["rise"]
	setDT = curSunD["set"]
	#print("Rise ",riseDT, "; Set ", setDT, "; current ",currentDT)
...
def disFromMoon(currentDT, currentAlt, currentAz, earthObserver):
	#twoObjectDistanceExplict(ra1,dec1,ra2,dec2)
	degreeFunc =  lambda x: x.degrees if type(x) is skyAngle else x
