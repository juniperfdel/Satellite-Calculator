...
class TimeObj:
	...
	def __repr__(self) -> str:
		return "TimeObj(datetime=%r, skyfield.timelib.Time=%r, numpy.datetime64=%r, astropy.time.Time=%r, pandas.Timestamp=%r)" % (self.dt, self.sf, self.np, self.ap, self.pd)
	...
	def get_start_day(self):
		return TimeObj(datetime.combine(self.dt.date(), datetime.min.time()), in_local_tz=self.local_tz)
	...
	def get_off_center(self, **kwargs: int):
		...
		return \
			TimeObj(self.dt - time_d, in_local_tz=self.local_tz), TimeObj(self.dt + time_d, in_local_tz=self.local_tz)
...
def get_off_list(t_start: TimeObj, t_end: TimeObj, t_step: TimeDeltaObj, final_type: int = 0) \
		-> Union[AstropyTimeSeries, np.array, List[datetime], SkyfieldTime, pandas.DataFrame]:
	"""
	...
	if final_type in {3, 4}:
		...
		ap_ts = AstropyTimeSeries(time_start=t_start.ap, time_delta=ap_step, n_samples=n_steps)
		if final_type == 3:
			return ap_ts
		else:
			return ap_ts.to_pandas()
	...
	raise TypeError("Please select a type 0 (numpy.datetime64), 1 (datetime), 2 (skyfield.timelib.Time), 3 (astropy.time.Time), or 4 (pandas.DataFrame)! ")


def today() -> TimeObj:
	dt_today = datetime.combine(datetime.today().date(), datetime.min.time())
