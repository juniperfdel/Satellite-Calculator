...
from utils import two_object_distance, SkyfieldConstants, TimeObj, MetaFormatter, Observatories
...
class SatellitePosition(metaclass=MetaFormatter):
	def __init__(self, in_obs_sat, in_time: TimeObj):
		...
		self.twilight_checker = dark_twilight_day(SkyfieldConstants.ephemeris, self.obs.sf)
		self.time = in_time
	...
	def sat_ra_dec(self) -> Tuple[float, float]:
		sat_ra, sat_dec, _ = self._sat_toc.radec(epoch=SkyfieldConstants.timescale.J2000)
	...
	def moon_ra_dec(self) -> Tuple[float, float]:
		moon_ra, moon_dec, _ = self.obs.sf_vector_sum.at(self.time.sf) \
			.observe(SkyfieldConstants.moon).apparent() \
	...
	def moon_distance(self) -> float:
		return two_object_distance(self.sat_ra, self.sat_dec, self.moon_ra, self.moon_dec)
	...
	def sat_lat_long(self) -> Tuple[float, float]:
		return self.sat_geo_position.latitude._degrees, self.sat_geo_position.longitude._degrees
