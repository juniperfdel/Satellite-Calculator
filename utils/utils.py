...
from utils import SkyfieldConstants, DayTransits, \
	Observatories, ObservatorySatellite, TimeDeltaObj, TimeObj, today, get_off_list


def get_day_transits(
		in_obs_sat: ObservatorySatellite, start_time: TimeObj, end_time: TimeObj, input_alt: float
) -> Sequence[DayTransits]:
	return find_sat_events(in_obs_sat, start_time, end_time, input_alt)


def get_obs_coord_between(
		in_obs_sat: ObservatorySatellite, start_time: TimeObj, end_time: TimeObj, time_step: TimeDeltaObj
) -> pandas.DataFrame:
...
def build_obs_sat(reload_sat_: bool, use_all_: bool = False) -> List[ObservatorySatellite]:
