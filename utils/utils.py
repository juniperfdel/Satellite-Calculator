...
def build_sat_list(
    ...
    use_all_: bool = False,
    local_tle: Union[Path, None] = None,
) -> List[EarthSatellite]:
    ...
    if cache_sat and (local_tle is None):
        ...
        cache_loc = Path(
            SkyfieldConstants.load.path_to(f"{today_utc.get_file_format()}_sats.txt")
        )
...
def build_obs_sat(
    reload_sat_: bool, cache_sat_: bool = False, use_all_: bool = False
) -> List[ObservatorySatellite]:
