...
from utils import (DayTransits, SingleCulmination, build_obs_sat,
                   get_day_transits, today)
...
def write_to_csv_file(in_file_name: str, in_config_section: str, in_days: List[DayTransits], ignore_day: bool, ignore_day_rising: bool, ignore_day_set: bool) -> None:
	...
	with open(in_file_name, "w") as fp:
		...
		for day_transits in in_days:
			for single_culmination in day_transits:
				...
				row = [str(csv_column.get_value(single_culmination)) for csv_column in good_col_defs]
				row_s = ",".join(row) + "\n"
				fp.write(row_s)
...
def main(in_args) -> None:
	today_utc = today()
	today_utc.set_local_timezone((get_localzone() if in_args.timezone == "local" else pytz.timezone(in_args.timezone)).zone)
	...
	for sat_obs in sat_obs_objects:
		print(f"Looking for {sat_obs.sat_name} at {sat_obs.obs_name} from {today_utc} to {final_day}")
	...
	write_to_csv_file(f"{in_args.output_file[0]}.csv", in_args.csv, culmination_data, in_args.ignore_daytime, in_args.ignore_rise_day, in_args.ignore_set_day)
...
if __name__ == "__main__":
	...
	parser.add_argument(
		'days', type=int, nargs='?', default=120, help='The number of days to calculate for, default of 120 days'
	)

	parser.add_argument(
		'-tz', '--timezone', type=str, default="local",
	...
	parser.add_argument('-ca', '--cache', action='store_true', help="Automatically Cache TLE files")
	parser.add_argument(
		'-a', '--alt', nargs='+', type=float, default=30.,
		help="Change the altitude at which the calculations for rising and falling occur"
	)

	parser.add_argument(
		'-c', '--csv', type=str, default="ALL",
		help="Specifies the section in csv_config.ini to use when building the csv files"
	)

	parser.add_argument(
		'-i', '--ignore-daytime', action='store_true', help='ignore the data points which occur during the day'
	)
	
	parser.add_argument(
		'-ir', '--ignore-rise-day', action='store_true', help='ignore the data points for which the rising occurs during the day'
	)

	parser.add_argument(
		'-is', '--ignore-set-day', action='store_true', help='ignore the data points for which the setting occurs during the day'
	)
